function fish_prompt --description 'Write out the prompt'
    ...
    switch "$USER"
        case root toor
            echo ┌[(date +%H:%M:%S)] "$root_color""$USER" "$path_color""$PWD" "$warning_color"["$oldstatus"]"$gray" [fish]"$normal_color"
            echo ╘■" "

        case '*'
            echo ┌[(date +%H:%M:%S)] "$user_color""$USER" "$path_color""$PWD" "$warning_color"["$oldstatus"]"$gray" [fish]"$normal_color"
            echo └▶" "
    end
end
